...
import { getModels, sendMessageToApi, createChat, uploadFile, updateChat, getChatMessages } from "../api/api";
...
export default function Homepage() {
	...
	async function sendMessage(updatedMessages) {
		if (message) {
			let chatId;
			setMessage("");
			...
			await sendMessageToApi(updatedMessages, currentModel, chatId).then((data) => {
				updatedMessages.push(data.choices[0].message);
				updateChat(chatId, updatedMessages, currentModel);
			});
		...
		<div className="flex flex-col items-center justify-center h-screen w-full">
			...
			<div className="w-1/2">
				{messages.map((msg, index) => (
					<div
						key={index}
						className={`${msg.role === "user" ? "bg-blue-200" : "bg-green-200"} p-2 m-2 rounded-xl`}
					>
						{msg.content}
					</div>
				))}
